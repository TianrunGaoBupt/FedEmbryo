...
from mains.embryo_pretrain.federated_new_adaption_runner import *
...
if __name__ == '__main__':
    ...
    # clients
    clients = []
    len_client = len(os.listdir(f'output/0124/fed_emb_pretrain_intra_inter_adaption_resnet50_raw-copy/exp-pid/tasks/'))
    for idx in range(len_client):
        label_info = json_load(f'output/0124/fed_emb_pretrain_intra_inter_adaption_resnet50_raw-copy/exp-pid/tasks/client{idx}/label_info.json')
        config = {
            'task': '0124/fed_emb_pretrain_intra_inter_adaption_resnet50_raw',
            'id_base': 'pid',
    ...
    if clients[0]['train']:
        ...
        for i in range(ROUNDS):
            if config['inter_dwa']:
                ...
                print('intra-dwa mode')
                # Train models
                local_weights = []
                for idx in range(len_client):
                    w, _ = run(i, idx, clients[idx], model_for_train=copy.deepcopy(local_model_list[idx]))
                    local_weights.append(copy.deepcopy(w))
                # Aggregate the model
                local_weights_list = average_weights(local_weights)
        ...
        if clients[0]['external_test']:
            ...
            for file in os.listdir(f'output/0124-ex'):
                ...
                label_info_test = json_load(f'output/0124-ex/{file}/fed_emb_pretrain_intra_inter_adaption_resnet50_raw-copy/exp-pid/tasks/label_info.json')
                for idx in range(len_client):
                    clients[idx]['task'] = f'0124-ex/{file}/fed_emb_pretrain_intra_inter_adaption_resnet50_raw'
