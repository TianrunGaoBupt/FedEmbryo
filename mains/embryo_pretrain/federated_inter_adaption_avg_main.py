...
from mains.embryo_pretrain.federated_inter_adaption_runner import *
...
if __name__ == '__main__':
    ...
    # clients
    clients = []
    len_client = len(os.listdir(f'output/0124/fed_emb_pretrain_inter_adaption_resnet50_raw/exp-pid/tasks/'))
    for idx in range(len_client):
        label_info = json_load(f'output/0124/fed_emb_pretrain_inter_adaption_resnet50_raw/exp-pid/tasks/client{idx}/label_info.json')
        config = {
            'task': '0124/fed_emb_pretrain_inter_adaption_resnet50_raw',
            'id_base': 'pid',
    ...
    if clients[0]['train']:
        ...
        if clients[0]['external_test']:
            ...
            for file in os.listdir(f'output/0124-ex'):
                ...
                label_info_test = json_load(f'output/0124-ex/{file}/fed_emb_pretrain_inter_adaption_resnet50_raw/exp-pid/tasks/label_info.json')
                for idx in range(len_client):
                    clients[idx]['task'] = f'0124-ex/{file}/fed_emb_pretrain_inter_adaption_resnet50_raw'
