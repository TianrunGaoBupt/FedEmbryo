...
def run(round, idx, config, model_for_train, device=torch.device('cuda')):
    ...
    if train:
        ...
        for pth in os.listdir(f'models/0124-ex/unseen_external/fed_emb_pretrain_resnet50_raw-exp-pid'):
            print(pth)
            if '.pkl' in pth:
                continue
            model_for_train.load_state_dict(torch.load(f'models/0124-ex/unseen_external/fed_emb_pretrain_resnet50_raw-exp-pid/{pth}', torch.device('cpu')))
...
def run_val(round, idx, config, model_for_train, device=torch.device('cuda')):
    ...
    label_info_test = json_load(f'output/0124-ex/unseen_external/fed_emb_pretrain_resnet50_raw/exp-pid/tasks/label_info.json')
    config_val = copy.deepcopy(config)
    config_val['task'] = '0124-ex/unseen_external/fed_emb_pretrain_resnet50_raw'
...
def run_ex_test(file_name, round, idx, config, model_for_train, device=torch.device('cuda')):
    ...
    for pth in os.listdir(f'models/0124-ex/unseen_external/fed_emb_pretrain_resnet50_raw-exp-pid'):
        ...
        if config['external_test']:
            ensure_path(f'output/0124/fed_emb_pretrain_resnet50_raw/exp-pid/results/{name_without_extension}')
            test_df_tmp.to_csv(f'output/0124/fed_emb_pretrain_resnet50_raw/exp-pid/results/{name_without_extension}/client_ex_{file_name}_{idx}.csv', index=False)
        else:
            ensure_path(f'output/{task_path}/results/{name_without_extension}')
