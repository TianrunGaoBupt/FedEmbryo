...
def run(round, idx, config, model_for_train, device=torch.device('cuda')):
    ...
    if train:
        ...
        for epoch in range(num_train_epochs):
            with Benchmark(f'Round {round}, Epoch {epoch}'):
                ...
                if epoch == 0 or epoch ==1:
                    ...
                    for i in range(len(task_weight)):
                        lambda_weight[i, epoch] = len(n_classes) * np.exp(task_weight[i] / 2) / sum([np.exp(task_weight[j] / 2) for j in range(len(task_weight))])
                ...
                with tqdm(train_dl, leave=False, file=sys.stdout) as t:
                    for batch in t:
                        ...
                        if epoch == 0 or epoch ==1:                          
                            ...
                            loss_return.append(loss_bp.item() / len(t))
                        else:
                            loss_bp = np.array([sum([lambda_weight[i, epoch] * torch.mean(sub_loss[i]).item() for i in range(len(task_weight))])])
                            loss_return.append(loss_bp / len(t))
                            loss_bp = torch.from_numpy(loss_bp).requires_grad_(True)
                        ...
                        for i in range(len(sub_loss)):
                            sub_task_loss.append(torch.mean(sub_loss[i]).item())
                        avg_cost[epoch, :len(task_weight)] += np.array(sub_task_loss) / len(t)

                        t.set_postfix(loss=float(loss_bp))
                        label = [l[~torch.isnan(l)] for l in label]
                        y = [yy[~torch.isnan(l)] for yy, l in zip(y, label)]
                        
                        # records['train-loss-list'].append(loss_bp)
                        # records['train-loss-list'].append([float(x) for x in loss_bp])
        ...
        for pth in os.listdir(f'models/{name}'):
            ...
            if '.pkl' in pth:
                continue
            model_for_train.load_state_dict(torch.load(f'models/{name}/{pth}', torch.device('cpu')))
...
def run_val(round, idx, config, model_for_train, device=torch.device('cuda')):
    ...
    label_info_test = json_load(f'output/0608-ex/fed_emb_pretrain_resnet50_raw/exp-pid/tasks/label_info.json')
    config_val = copy.deepcopy(config)
    config_val['task'] = '0608-ex/fed_emb_pretrain_resnet50_raw'
...
def run_ex_test(round, idx, config, model_for_train, device=torch.device('cuda')):
    ...
    for pth in os.listdir(f'models/{name}'):
        ...
        if config['external_test']:
            ensure_path(f'output/0608/fed_emb_pretrain_resnet50_raw/exp-pid/results/{name_without_extension}')
            test_df_tmp.to_csv(f'output/0608/fed_emb_pretrain_resnet50_raw/exp-pid/results/{name_without_extension}/client_ex{idx}.csv', index=False)
        else:
            ensure_path(f'output/{task_path}/results/{name_without_extension}')
            test_df_tmp.to_csv(f'output/{task_path}/results/{name_without_extension}/client{idx}.csv', index=False)
