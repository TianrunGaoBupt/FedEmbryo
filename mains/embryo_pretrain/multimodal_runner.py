...
def run(config, device=torch.device('cuda')):
    ...
    if config['external_test']:
        df_ex = pd.read_csv(f'output/0628-ex/d56_external/outcomes_reverse_multimodal_resnet50_raw-2000/exp-pid/tasks/data.csv', low_memory=False)
        test_df = df_ex.copy()
    else:
    ...
    for saver in savers:
        ...
        if config['external_test']:
            test_df_tmp.to_csv(f'output/{task_path}/results/{saver.key}/mt_fold_ex_d56_external-2000.csv', index=False)
        else:
            test_df_tmp.to_csv(f'output/{task_path}/results/{saver.key}/mt_fold_test.csv', index=False)
    model.predict = False
