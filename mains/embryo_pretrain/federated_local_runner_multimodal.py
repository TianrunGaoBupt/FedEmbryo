...
def run(config, device=torch.device('cuda')):
    ...
    task_file_path = f'output/{task_path}/tasks/client0/data.csv'
    pkl_dump(config, f'output/{task_path}/tasks/client0/local_config.pkl')

    label_info = json_load(f'output/{task_path}/tasks/client0/label_info.json')
    ...
    if config['external_test']:
        df_ex = pd.read_csv(f'output/0628-ex/unseen_external/fed_outcomes_reverse_multimodal_resnet50_raw/exp-pid/tasks/data.csv', low_memory=False)
        test_df = df_ex.copy()
    else:
    ...
    for saver_init in savers_init:
        savers.append(ModelSaver(model, f'models/0628/loc_outcomes_multimodal_resnet50_raw-exp-pid-client0', records, saver_init[0], saver_init[1]))
    savers.append(ModelSaverOneEpoch(model, f'models/0628/loc_outcomes_multimodal_resnet50_raw-exp-pid-client0', records, epoch=num_train_epochs - 1))
    if train:
    ...
    for saver in savers:
        ...
        if config['external_test']:
            test_df_tmp.to_csv(f'output/{task_path}/results/{saver.key}/loc_client_ex_unseen_external_0.csv', index=False)
        else:
            test_df_tmp.to_csv(f'output/{task_path}/results/{saver.key}/loc_client0.csv', index=False)
    model.predict = False
