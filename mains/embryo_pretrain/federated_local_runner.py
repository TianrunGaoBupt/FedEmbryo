...
def run(config, device=torch.device('cuda')):
    ...
    if config['external_test']:
        df_ex = pd.read_csv(f'output/0124-ex/unseen_external/fed_emb_pretrain_resnet50_raw/exp-pid/tasks/data.csv', low_memory=False)
        test_df = df_ex.copy()
    else:
    ...
    train_pro = DataLoader(train_ds, sampler=SequentialSampler(train_ds), batch_size=batch_size, num_workers=32)
    ...
    for saver_init in savers_init:
        savers.append(ModelSaver(model, f'models/0124/loc_emb_pretrain_resnet50_raw-exp-pid-client0', records, saver_init[0], saver_init[1]))
    savers.append(ModelSaverOneEpoch(model, f'models/0124/loc_emb_pretrain_resnet50_raw-exp-pid-client0', records, epoch=num_train_epochs - 1))
    if train:
    ...
    for saver in savers:
        ...
        if config['external_test']:
            test_df_tmp.to_csv(f'output/{task_path}/results/{saver.key}/loc_client_ex_d56_external_0.csv', index=False)
        else:
            test_df_tmp.to_csv(f'output/{task_path}/results/{saver.key}/loc_internal_client0.csv', index=False)
    model.predict = False
