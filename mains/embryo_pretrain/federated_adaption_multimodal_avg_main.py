...
from mains.embryo_pretrain.federated_multimodal_runner import *
...
if __name__ == '__main__':
    ...
    # clients
    clients = []
    len_client = len(os.listdir(f'output/0124/fed_outcomes_adaption_reverse_multimodal_resnet50_raw/exp-pid/tasks/'))
    for idx in range(len_client):
        label_info = json_load(f'output/0124/fed_outcomes_adaption_reverse_multimodal_resnet50_raw/exp-pid/tasks/client{idx}/label_info.json')
        config = {
            'task': '0124/fed_outcomes_adaption_reverse_multimodal_resnet50_raw',
            'id_base': 'pid',
    ...
    if clients[0]['train']:
        ...
        for i in range(ROUNDS):
            ...
            # avg_cost[i, :len_client] += np.array(local_loss)[:,0]
        ...
        if clients[0]['external_test']:
            ...
            for file in os.listdir(f'output/0124-ex'):
                ...
                label_info_test = json_load(f'output/0124-ex/{file}/fed_outcomes_adaption_reverse_multimodal_resnet50_raw/exp-pid/tasks/label_info.json')
                for idx in range(len_client):
                    clients[idx]['task'] = f'0124-ex/{file}/fed_outcomes_adaption_reverse_multimodal_resnet50_raw'
