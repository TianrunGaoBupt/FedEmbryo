...
def run(round, idx, config, model_for_train, device=torch.device('cuda')):
    ...
    if train:
            ...
            with Benchmark(f'Round {round}, Epoch {epoch}'):
                ...
                with tqdm(train_dl, leave=False, file=sys.stdout) as t:
                    for batch in t:
                        ...
                        clinical_factor = batch['clinical_factor'].to(device)
                        # end clinical_factor
                        label = [l.to(device) for l in batch['label']]
                        optimizer.zero_grad()
                        y, loss = model_for_train(img, clinical_factor, label)
                ...
                if os.path.exists(f'models/{name}/best_valid-loss.pkl'):
                    ...
                    # print(f'client_{idx} last round best valid loss:', records[f'best_valid-loss'])
                else:
        ...
        for pth in os.listdir(f'models/0124-ex/unseen_external/fed_outcomes_mlp_reverse_multimodal_resnet50_raw-exp-pid'):
            print(pth)
            if '.pkl' in pth:
                continue

            model_for_train.load_state_dict(torch.load(f'models/0124-ex/unseen_external/fed_outcomes_mlp_reverse_multimodal_resnet50_raw-exp-pid/{pth}', torch.device('cpu')))
            ...
            with torch.no_grad():
                with tqdm(test_dl, leave=False, file=sys.stdout) as t:
                    for batch in t:
                        ...
                        clinical_factor = batch['clinical_factor'].to(device)
                        # end clinical_factor
                        y = model_for_train(img, clinical_factor)
...
def run_val(round, idx, config, model_for_train, device=torch.device('cuda')):
    ...
    label_info_test = json_load(f'output/0124-ex/unseen_external/fed_outcomes_mlp_reverse_multimodal_resnet50_raw/exp-pid/tasks/label_info.json')
    config_val = copy.deepcopy(config)
    config_val['task'] = '0124-ex/unseen_external/fed_outcomes_mlp_reverse_multimodal_resnet50_raw'
...
def run_ex_test(file_name, round, idx, config, model_for_train, device=torch.device('cuda')):
    ...
    for pth in os.listdir(f'models/0124-ex/unseen_external/fed_outcomes_mlp_reverse_multimodal_resnet50_raw-exp-pid'):
        ...
        if config['external_test']:
            ensure_path(f'output/0124/fed_outcomes_mlp_reverse_multimodal_resnet50_raw/exp-pid/results/{name_without_extension}')
            test_df_tmp.to_csv(f'output/0124/fed_outcomes_mlp_reverse_multimodal_resnet50_raw/exp-pid/results/{name_without_extension}/client_ex_{file_name}_{idx}.csv', index=False)
        else:
            ensure_path(f'output/{task_path}/results/{name_without_extension}')
